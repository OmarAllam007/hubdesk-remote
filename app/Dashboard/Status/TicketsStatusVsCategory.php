...
class TicketsStatusVsCategory
{
    ...
    function process()
    {
        ...
        $ticketStatusData->put('statuses', $data);
    ...
    function fillData()
    {
        return $this->data->whereNotNull($this->date)
            ...
            ->groupBy(function ($ticket) {
                return $ticket->Status ? $ticket->Status : 'Not Assigned';
    ...
    private function fillFooter($data)
    {
        ...
        $data->each(function ($status, $key) use ($chartData, $footerRow) {
            $chartData->put($key, $status['Total']);

            $status['items']->each(function ($item, $itemKey) use ($footerRow) {
