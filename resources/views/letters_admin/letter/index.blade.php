...
@section('header')
    ...
    <div class="heading-actions pull-right">
        <form action="" class="form-inline" method="get">
            ...
            <a href="{{ route('letters.letter.create') }} " class="btn btn-sm btn-primary"><i
            ...
            @if ($letters->total())
                <table class="listing-table">
                    ...
                    @foreach($letters as $letter)
                        <tr>
                            <td class="col-md-5"><a
                                        href="{{ route('letters.letter.show', $letter) }}">{{ $letter->name }}</a>
                            </td>
                            {{--                        <td>{{$letter->is_disabled ? 'No' : 'Yes'}}</td>--}}
                            <td class="col-md-3">
                                <form action="{{ route('letters.letter.destroy', $letter) }}" method="post">
                                    {{csrf_field()}} {{method_field('delete')}}
                                    <a class="btn btn-sm btn-primary"
                                       href="{{ route('letters.letter.edit', $letter) }} "><i
                ...
                {!! $letters->links() !!}
                {{--            @include('partials._pagination', ['items' => $letters])--}}
            @else
