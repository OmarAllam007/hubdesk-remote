...
@section('header')
    ...
    <div class="heading-actions pull-right">
        <form action="" class="form-inline" method="get">
            ...
            <a href="{{ route('letters.signature.create') }} " class="btn btn-sm btn-primary"><i
        ...
        <section class="w-3/4 mx-5 ">
            @if ($signatures->total())
                <div class="bg-white shadow-md rounded my-5 ">
                    <table class="min-w-max w-full table-auto">
                        ...
                        <tr>
                            <th class="py-5  px-6 text-left">{{t('Letter')}}</th>
                            <th class="py-5  px-6 text-left">{{t('User')}}</th>
                        ...
                        @foreach($signatures as $signature)
                            <tr class="border-b border-gray-200 hover:bg-yellow-100">
                                <td class="py-5  px-6 text-left whitespace-nowrap">{{ $signature->letter->name }}
                                </td>

                                <td class="py-5  px-6 text-left whitespace-nowrap">{{ $signature->user->name }}</td>
                                <td class="py-5  px-6 text-left whitespace-nowrap">{{ $signature->business_unit->name }}</td>
                                <td class="py-5  px-6 text-left whitespace-nowrap">
                                    <form action="{{ route('letters.signature.destroy', $signature) }}" method="post">
                                        {{csrf_field()}} {{method_field('delete')}}
                                        <a class="btn btn-sm btn-primary"
                                           href="{{ route('letters.signature.edit', $signature) }} "><i
            ...
            @else
                <div class="flex bg-yellow-100 p-5 rounded-xl  shadow-sm m-5">
                    <i class="fa fa-exclamation-circle px-2    "></i> <strong>No Signatures found</strong></div>
            @endif
        </section>
@stop
