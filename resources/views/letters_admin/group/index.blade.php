...
@section('header')
    ...
    <div class="heading-actions pull-right">
        <form action="" class="form-inline" method="get">
            ...
            <a href="{{ route('letters.letter-group.create') }} " class="btn btn-sm btn-primary"><i
        ...
        @if ($groups->total())
            <table class="listing-table">
                ...
                @foreach($groups as $group)
                    <tr>
                        <td class="col-md-5"><a href="{{ route('letters.letter-group.show', $group) }}">{{ $group->name }}</a>
                        </td>
{{--                        <td>{{$group->is_disabled ? 'No' : 'Yes'}}</td>--}}
                        <td class="col-md-3">
                            <form action="{{ route('letters.letter-group.destroy', $group) }}" method="post">
                                {{csrf_field()}} {{method_field('delete')}}
                                <a class="btn btn-sm btn-primary" href="{{ route('letters.letter-group.edit', $group) }} "><i
        ...
        @else
            <div class="flex bg-yellow-100 p-5 rounded-xl  shadow-sm m-5">
                <i class="fa fa-exclamation-circle px-2    "></i> <strong>No groups found</strong></div>
        @endif
    </section>
@stop
