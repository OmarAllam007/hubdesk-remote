...
@section('header')
    <h4 class="pull-left">{{t('Priority')}}</h4>
    <form action="" class="form-inline" method="get">
        ...
        <a href="{{ route('admin.priority.create') }} " class="btn btn-sm btn-primary"><i class="fa fa-plus"></i></a>
...
@section('body')
    <div class="flex w-full">
        ...
        <section class="w-3/4">
            @if ($priorities->total())
                <table class="listing-table">
                    ...
                    @foreach($priorities as $priority)
                        <tr>
                            <td class="col-md-5"><a
                                        href="{{ route('admin.priority.edit', $priority) }}">{{ $priority->name }}</a>
                            </td>
                            <td class="col-md-3">
                                <form action="{{ route('admin.priority.destroy', $priority) }}" method="post">
                                    {{csrf_field()}} {{method_field('delete')}}
                                    <a class="btn btn-sm btn-primary"
                                       href="{{ route('admin.priority.edit', $priority) }} "><i class="fa fa-edit"></i>
                                        Edit</a>
            ...
            @else
                <div class="alert alert-info"><i class="fa fa-exclamation-circle"></i> <strong>No priority
                        found</strong></div>
            @endif
        </section>
@stop
