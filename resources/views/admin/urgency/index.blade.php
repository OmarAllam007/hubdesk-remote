...
@section('header')
    ...
    <form action="" class="form-inline" method="get">
        ...
        <a href="{{ route('admin.urgency.create') }} " class="btn btn-sm btn-primary pull-right"><i
...
@section('body')
    <div class="flex w-full">
        ...
        <section class="w-3/4">
            @if ($urgencies->total())
                <table class="listing-table">
                    ...
                    @foreach($urgencies as $urgency)
                        <tr>
                            <td class="col-md-5"><a
                                        href="{{ route('admin.urgency.edit', $urgency) }}">{{ $urgency->name }}</a></td>
                            <td class="col-md-3">
                                <form action="{{ route('admin.urgency.destroy', $urgency) }}" method="post">
                                    {{csrf_field()}} {{method_field('delete')}}
                                    <a class="btn btn-sm btn-primary"
                                       href="{{ route('admin.urgency.edit', $urgency) }} "><i class="fa fa-edit"></i>
                                        Edit</a>
            ...
            @else
                <div class="alert alert-info"><i class="fa fa-exclamation-circle"></i> <strong>No urgency found</strong>
                </div>
            @endif
        </section>
@stop
