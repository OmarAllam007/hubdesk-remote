...
@section('body')
    <div class="flex w-full">
        ...
        <section class="w-3/4">
            @if ($regions->total())
                <table class="listing-table">
                    ...
                    @foreach($regions as $region)
                        <tr>
                            <td class="col-md-8"><a href="{{route('admin.region.edit', $region)}}">{{$region->name}}</a>
                            </td>
                            <td class="col-md-4">
                                <a class="btn btn-sm btn-primary" href="{{route('admin.region.edit', $region)}}"><i
                                            class="fa fa-edit"></i> Edit</a>
                                <form action="{{route('admin.region.destroy', $region)}}" method="post"
                                      class="inline-block">
            ...
            @else
                <div class="alert alert-info"><i class="fa fa-exclamation-circle"></i> <strong>No regions found</strong>
                </div>
            @endif
        </section>
@stop