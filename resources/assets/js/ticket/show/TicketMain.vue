      ...
      <div class="m-3 mt-10">
        ...
        <div class="w-full mt-3 shadow-md">
          <div class="flex justify-between">
            <div class="w-1/4 p-3 bg-gray-800 bg-opacity-75 text-white rounded-tl-sm   font-bold">
              {{ $root.t('Category') }}
            </div>
            <div class="w-1/4 p-3 bg-white ">{{ $root.t(ticket.category) }}</div>
            <div class="w-1/4 p-3 bg-gray-800  bg-opacity-75 text-white font-bold">{{ $root.t('Group') }}</div>
            <div class="w-1/4 p-3 bg-white rounded-tr-xl ">{{ $root.t(ticket.group) }}</div>
          </div>
          <div class="flex justify-between">
            <div class="w-1/4 p-3 bg-gray-800  bg-opacity-75 text-white  font-bold">{{
                $root.t('Subcategory')
              }}
            </div>
            <div class="w-1/4 p-3 bg-white ">{{ $root.t(ticket.subcategory) }}</div>
            <div class="w-1/4 p-3 bg-gray-800     bg-opacity-75 text-white font-bold"> {{ $root.t('Technician') }}
            </div>
            <div class="w-1/4 p-3 bg-white ">{{ $root.t(ticket.technician) }}</div>
          </div>

          <div class="flex justify-between">
            <div class="w-1/4 p-3 bg-gray-800 bg-opacity-75 text-white  font-bold"> {{ $root.t('Item') }}</div>
            <div class="w-1/4 p-3 bg-white ">{{ $root.t(ticket.item) }}</div>
            <div class="w-1/4 p-3 bg-gray-800     bg-opacity-75 text-white font-bold"> {{ $root.t('Priority') }}
            </div>
            <div class="w-1/4 p-3 bg-white ">{{ $root.t(ticket.priority) }}</div>
          </div>

          <div class="flex justify-between">
            <div class="w-1/4 p-3 bg-gray-800  bg-opacity-75 text-white  font-bold">{{ $root.t('SubItem') }}</div>
            <div class="w-1/4 p-3 bg-white ">{{ $root.t(ticket.subItem) }}</div>
            <div class="w-1/4 p-3 bg-gray-800     bg-opacity-75 text-white font-bold">{{ $root.t('Service Cost') }}
            </div>
            <div class="w-1/4 p-3 bg-white ">{{ $root.t(ticket.service_cost) }}</div>
          </div>

          <div class="flex justify-between">
            <div class="w-1/4 p-3 bg-gray-800  bg-opacity-75 text-white  font-bold">{{ $root.t('SLA') }}</div>
            <div class="w-1/4 p-3 bg-white ">{{ $root.t(ticket.sla) }}</div>
            <div class="w-1/4 p-3 bg-gray-800  bg-opacity-75 text-white font-bold">{{ $root.t('Due Time') }}</div>
        ...
        <div class="w-full mt-3 shadow-md">
          ...
          <div class="flex justify-between">
            <div class="w-1/4 p-3 bg-gray-800     bg-opacity-75 text-white  font-bold">{{ $root.t('Department') }}
            </div>
            <div class="w-1/4 p-3 bg-white ">{{ $parent.data.requester.department }}</div>
            <div class="w-1/4 p-3 bg-gray-800     bg-opacity-75 text-white font-bold">{{ $root.t('Job title') }}
            </div>
            <div class="w-1/4 p-3 bg-white ">{{ $parent.data.requester.job_title }}</div>
          </div>

          <div class="flex justify-between">
            <div class="w-1/4 p-3 bg-gray-800     bg-opacity-75 text-white  font-bold">{{ $root.t('Email') }}</div>
            <div class="w-1/4 p-3 bg-white ">{{ $parent.data.requester.email }}</div>
            <div class="w-1/4 p-3 bg-gray-800     bg-opacity-75 text-white font-bold">{{ $root.t('Employee ID') }}
            </div>
          ...
          <div class="flex justify-between">
            <div class="w-1/4 p-3 bg-gray-800     bg-opacity-75 text-white  font-bold">{{ $root.t('Mobile') }}</div>
            <div class="w-1/4 p-3 bg-white ">{{ $parent.data.requester.mobile1 }}</div>
            <div class="w-1/4 p-3 bg-gray-800     bg-opacity-75 text-white font-bold">{{ $root.t('Phone') }}</div>
