      ...
      <div class="flex flex-col w-full  p-5 my-5  bg-white rounded-xl shadow-md" v-for="section in fields">
        ...
        <div class="flex flex-wrap w-full  mx-2  my-5 rounded-xl">
          <div v-for="item in section.fields" class="w-full md:w-1/2 lg:w-1/2 xl:w-1/2 px-1 ">
            <div v-if="item.id == 860 || item.id == 687">
...
import ReplacementComponent from "./_components/ReplacementComponent";
...
export default {
  ...
  mounted() {
    EventBus.$on('replacement-user-from-sap', (value) => {
      this.form.custom_fields[value.id] = value.value.employee_id
      this.form.custom_fields['Replacement Position'] = value.value.en_occupation ? value.value.en_occupation : 'N/A'
      this.form.custom_fields['Replacement Date of Join'] = value.value.date_of_join
      this.form.custom_fields['Replacement Separation date'] = value.value.is_active ? 'N/A' : value.value.end_of_service_date
    });
  },
  methods: {
    changeReplacement(value) {
      console.log(value)
    },
    ...
    submitTicket() {
      ...
      axios.post(`/ticket/post_recruitment_requisition_ticket`,
          ticket, {
            headers: {
